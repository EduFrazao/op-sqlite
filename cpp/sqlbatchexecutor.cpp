...
void toBatchArguments(jsi::Runtime &rt, jsi::Array const &batchParams,
  ...
  for (int i = 0; i < batchLength; i++) {
    ...
    if (!commandParams.isUndefined() &&
       ...
       * sql command. This structure can be optmized to reuse prepared
       * statements. That is the idea of this method signature. EG: sql | [[]] -
       * Same SQL, Many different binded values.
       */
      ...
      auto params = std::make_shared<BatchParams>(BatchParams(stmtParams));
      commands->push_back({query, params});
    } else {
      auto params = std::make_shared<BatchParams>(
          BatchParams(toVariantVec(rt, commandParams)));
      commands->push_back({query, params});
    }
  }
}
...
BatchResult sqliteExecuteBatch(std::string dbName,
  ...
  try {
    ...
    for (int i = 0; i < commandCount; i++) {
      ...
      // Checking if
      if (std::holds_alternative<std::vector<std::vector<JSVariant>>>(
              *params)) {
        auto multiStatementParams =
            std::get<std::vector<std::vector<JSVariant>>>(*params);
        ...
        for (int x = 0; x < size; x++) {
          ...
          auto result =
              opsqlite_execute_prepared_statement(db, stmt, nullptr, nullptr);
